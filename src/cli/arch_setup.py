...
def install_python_dependencies(cuda_available=False):
    """Install required Python packages optimized for Arch Linux"""
    print("[INFO] Installing Python dependencies for Arch Linux...")
    
    # First upgrade pip
    run_command("pip install --upgrade pip setuptools wheel", "Upgrading pip")
    
    # For Arch Linux, we might want to try the Arch-specific approach
    # but we'll start with the standard pip approach
    if cuda_available:
        # Install PyTorch with CUDA support for Arch
...
def main():
    ...
    if args.all or args.install_deps:
        ...
        install_python_dependencies(cuda_available)
